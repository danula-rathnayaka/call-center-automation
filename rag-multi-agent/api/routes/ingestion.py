...
from multiagent_rag.utils.scrape_review_store import (get_pending_queue, get_queue_item, approve_item, reject_item,
                                                      get_stats, )
...
def _run_ingestion(file_path: str, label: str) -> IngestionResponse:
    result = ingestion_app.invoke({"file_path": file_path, "chunks": [], "document_hash": "", "status": "start", })
    status = result.get("status", "unknown")

    if status == "completed":
        chunks = result.get("chunks", [])
        return IngestionResponse(status="completed",
            message=f"Successfully ingested '{label}' into the knowledge base.", chunks_ingested=len(chunks), )
    elif status == "duplicate":
        return IngestionResponse(status="duplicate",
            message=f"'{label}' has already been ingested. Skipping to avoid duplication.", chunks_ingested=0, )
    elif status == "failed":
        return IngestionResponse(status="failed", message=f"Failed to extract content from '{label}'.",
            chunks_ingested=0, )
    else:
...
async def ingest_batch(files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        if ext not in ALLOWED_EXTENSIONS:
            results.append(
                IngestionResponse(status="failed", message=f"Unsupported file type '{ext}' for '{filename}'.",
                    chunks_ingested=0, ))
            failed += 1
            continue
        ...
        try:
            ...
            results.append(IngestionResponse(status="failed", message=f"Failed to save '{filename}': {str(e)}",
                chunks_ingested=0, ))
            failed += 1

    loop = asyncio.get_running_loop()
    ingestion_tasks = [loop.run_in_executor(None, _run_ingestion, path, label) for path, label in saved_paths]
    ingestion_results = await asyncio.gather(*ingestion_tasks, return_exceptions=True)

    for res in ingestion_results:
        if isinstance(res, Exception):
            results.append(IngestionResponse(status="failed", message=str(res), chunks_ingested=0, ))
            failed += 1
        else:
            results.append(res)
    ...
    return BatchIngestionResponse(results=results, total_files=len(files), successful=successful, failed=failed,
...
async def approve_scraped_page(item_id: int):
    try:
        ...
        result = await asyncio.get_event_loop().run_in_executor(None, _run_ingestion, item["url"], item["url"])

        approve_item(item_id)

        return IngestionResponse(status=result.status,
